...
// ── AVX2 Prefetch Multi-threaded (scaling curve) ────────────────────────────
//
// Parametrized by {n_users, n_threads}. Only 500M users — threading is
// irrelevant at L3-resident scale where thread creation overhead dominates.
// range(0) = n_users, range(1) = n_threads.

class MtEvalFixture : public benchmark::Fixture {
public:
    void SetUp(const benchmark::State& state) override {
        n_users_   = static_cast<size_t>(state.range(0));
...
BENCHMARK_DEFINE_F(MtEvalFixture, Avx2PrefetchMT)(benchmark::State& state) {
    for (auto _ : state) {
        eval_avx2_prefetch_mt(a_.get(), b_.get(), not_c_.get(), result_.get(),
                              n_words_, n_threads_);
        benchmark::ClobberMemory();
    }
    state.SetBytesProcessed(
...
BENCHMARK_REGISTER_F(MtEvalFixture, Avx2PrefetchMT)
    ->Args({500'000'000, 1})
    ->Args({500'000'000, 2})
    ->Args({500'000'000, 4})
    ->Args({500'000'000, 8})
